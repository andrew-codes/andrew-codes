...
- **Prompt engineering**: Iteratively refining the AI prompt to provide clearer instructions and examples for handling multiple intents in a single user utterance.
- **Intent classification via langchain agent**: Implementing a separate, custom intent classification [agent](https://www.langchain.com/) that can first categorize user speech into distinct intents before passing it to the main AI for execution. This could help ensure that each intent is handled appropriately, even when multiple intents are present.
