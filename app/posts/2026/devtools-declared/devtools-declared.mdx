...
Publishing a reversal four months later is awkward, so let me start there.
...
{/* IMAGE SUGGESTION: A fan-out diagram: source boxes on the left (home/AGENTS.md, home/.agents/skills, home/.pi/agent/agents, home/.config/mcp/mcp.json) with arrows to three harness boxes on the right (Claude Code, pi,
Codex), each arrow labeled with its mechanism, "symlink" or "merged on activation". It earns its place by showing at a glance which agent gets which piece, including the uneven overlap the paragraph above admits to, and
it sets up the next post. */}
